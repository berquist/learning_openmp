...
void make_coulomb(arma::mat& J, arma::mat& P, arma::vec& ERI) {
  J.zeros();
  for (size_t mu = 0; mu < P.n_rows; mu++) {
    for (size_t nu = 0; nu < P.n_cols; nu++) {
      double J_mu_nu = 0.0;
      int munu = idx2(mu, nu);
#pragma omp parallel for reduction(+:J_mu_nu)
      for (size_t lm = 0; lm < P.n_rows; lm++) {
        for (size_t sg = 0; sg < P.n_cols; sg++) {
          int lmsg = idx2(lm, sg);
          J_mu_nu += P(lm, sg) * ERI(idx2(munu, lmsg));
        }
      }
      J(mu, nu) = J_mu_nu;
    }
...
void make_exchange(arma::mat& K, arma::mat& P, arma::vec& ERI) {
  K.zeros();
  for (size_t mu = 0; mu < P.n_rows; mu++) {
    for (size_t nu = 0; nu < P.n_cols; nu++) {
      double K_mu_nu = 0.0;
#pragma omp parallel for reduction(+:K_mu_nu)
      for (size_t lm = 0; lm < P.n_rows; lm++) {
        int mulm = idx2(mu, lm);
        for (size_t sg = 0; sg < P.n_cols; sg++) {
          int nusg = idx2(nu, sg);
          K_mu_nu += P(lm, sg) * ERI(idx2(mulm, nusg));
        }
      }
      K(mu, nu) = K_mu_nu;
    }
...
void build_fock(arma::mat& F, arma::mat& P, arma::mat& H, arma::vec& ERI) {
  ...
  make_coulomb(J, P, ERI);
  make_exchange(K, P, ERI);
  F = H + 2*J - K;
}
